	...
	const logIn = async (event: React.FormEvent) => {
		...
		switch (response.status) {
			case HttpStatusCode.OK:
				...
				storage.setAccessToken(content.access_token);
	...
	return (
		<div className='flex flex-col items-center flex-grow bg-blue-50 p-3'>
			...
			<div className='flex flex-col bg-white w-full md:w-80 md:text-lg mt-12 md:mt-40 m-5 p-5 rounded-lg shadow-lg'>
				<input className='input mt-3 mb-2' type='text' name='email' placeholder='Enter email' onChange={emailChangeHandler} />
				<input className='input' type='password' name='password' placeholder='Enter password' onChange={passwordChangeHandler} />
				<p className='self-center text-red-500 text-center w-52 mt-3 mb-6'>{errorLabelText}</p>
