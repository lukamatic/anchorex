function ListItem(item: any, numberOfDays: number = 2) {
	...
	return (
		<div className='w-1/3 px-4 py-2' key={item.id}>
			<button className='p-3 rounded-lg w-full bg-white mt-2 shadow-md hover:shadow-2xl transition-transform duration-75 transform hover:scale-105 h-full flex flex-col'>
				<div className='flex flex-col flex-1'>
					...
					<div className='flex flex-row justify-between'>
						...
						<p className='text-gray-500 text-xs text-left'>💸 {regularPrice * numberOfDays}</p>
					</div>
				</div>
			</button>
		</div>
