...
const MonthlyCalendar = (props: { year: number; month: number }) => {
  const weeks = calendarUtil.generateWeeksInMonth(props.year, props.month);

  return (
    <div className='flex flex-col flex-grow md:m-8 border-2 border-blue-500 rounded-lg'>
      ...
      <div className='flex flex-col flex-grow'>
        {weeks.map((week, index) => {
          return <MonthlyCalendarDatesInWeek week={week} index={index} />;
        })}
      </div>
    </div>
  );
