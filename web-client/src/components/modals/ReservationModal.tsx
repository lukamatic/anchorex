...
const ReservationModal = (props: {
  ...
  const selectAdditionalService = (service: any) => {
    ...
    const index = selectedAdditionalServices.findIndex(
  ...
  const additionalServices = reservation?.services?.filter(
    (e: any) => e.type === 'ADDITIONAL'
  );
  ...
  const additionServiceContainer = () => (
    <div className='p-2 rounded-md border-2 border-solid border-blue-200 mt-5 w-80'>
      ...
      <p className='text-gray-300 mb-0 text-sm border-b border-solid border-blue-200 w-full pb-2'>
      ...
      {additionalServices?.map((service: any, index: number) => {
        ...
        return (
          <div
            key={index}
            className={`cursor-pointer flex flex-row items-center mt-2 ${
              isSelected ? '' : ''
            }`}
            onClick={selectThisService}
          >
            ...
            <span className='ml-2 text-gray-300 text-xs'>
              ${service.price * numberOfPeopleToBook * numberOfDays}
            </span>
          </div>
        );
      })}
  ...
  const bookReservation = async () => {
    const price =
      numberOfDays * numberOfPeopleToBook * (regularPrice + additionalCost);
    const data = {
      ...
      price,
      services: [
        ...reservation?.services?.filter((e: any) => e.type === 'REGULAR'),
        ...selectedAdditionalServices,
      ],
                ...
                <div className='px-8 py-8'>
                  ...
                  <div className='flex flex-row'>
                    <div className='flex flex-col flex-1'>
                      ...
                      <p className='text-gray-500  text-left'>
                        Avg.score: {reservation.averageRating}
                      </p>
                    </div>
                    <div>
