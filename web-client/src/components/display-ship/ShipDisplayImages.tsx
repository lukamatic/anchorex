...
const ShipDisplayImages = () => {
  ...
  const uploadPhotos = () => {
    const url = `/api/ship/${params.id}/images/add`;
  ...
  const removeImage = async (imageId: number) => {
    const url = `/api/ship/images/remove/${imageId}`;
    const headers = [
      ['Authorization', 'Bearer ' + localStorageUtil.getAccessToken()],
    ];
  ...
  return (
    <div>
      ...
      <div className="container flex flex-col mx-auto w-full mt-12 ml-96 max-w-4xl">
        ...
        {authContext.user.role === UserRole.SHIP_OWNER && (
          <div className="mb-20">
            <input
              type="file"
...
export default ShipDisplayImages;
